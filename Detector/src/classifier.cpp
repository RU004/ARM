...
void NumberClassifier::extractNumbers(const cv::Mat & src, std::vector<Armor> & armors)
{
    ...
    for (auto & armor : armors) {
        ...
        cv::warpPerspective(src, number_image, rotation_matrix, cv::Size(warp_width, warp_height));  //得到正面观察视角的图片 number_image
...
void NumberClassifier::classify(std::vector<Armor> & armors)
{
    for (auto & armor : armors) {
        ...
        cv::Mat blob;
        cv::dnn::blobFromImage(image, blob,1.0,cv::Size(),cv::Scalar(),false,false,CV_32F);
